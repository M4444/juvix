...
metasT = \case
  ...
  CatProduct a b -> metasT a <> metasT b
  CatCoproduct a b -> metasT a <> metasT b
  CatProductIntro a b -> metasT a <> metasT b
  CatProductElimLeft b s -> metasT b <> metasT s
  CatProductElimRight a t -> metasT a <> metasT t
  CatCoproductIntroLeft a -> metasT a
  CatCoproductIntroRight b -> metasT b
  CatCoproductElim a b s t p -> foldMap metasT [a, b, s, t, p]
...
metasV = \case
  ...
  VCatProduct a b -> metasV a <> metasV b
  VCatCoproduct a b -> metasV a <> metasV b
  VCatProductIntro a b -> metasV a <> metasV b
  VCatProductElimLeft b s -> metasV b <> metasV s
  VCatProductElimRight a t -> metasV a <> metasV t
  VCatCoproductIntroLeft a -> metasV a
  VCatCoproductIntroRight b -> metasV b
  VCatCoproductElim a b s t p -> foldMap metasV [a, b, s, t, p]
  VUnitTy -> mempty
  VUnit -> mempty
  VNeutral n -> metasN n
...
data Env primTy primVal = Env
  { subst :: Subst primTy primVal,
    ...
    globals :: Core.Globals IR.T IR.T primTy primVal
  }
  deriving (Generic)

data Error primTy primVal
  = -- | metavariable occurs in its own solution
    Occurs MetaVar (Value primTy primVal)
  | -- | clash between two incompatible terms
    Clash (Value primTy primVal) (Value primTy primVal)
  | -- | clash between two incompatible /usages/
    ClashU Usage.T Usage.T
  | -- | reference to outside bound variable
    -- (unsupported until  higher-order unification is implemented)
    OutsideVariables (Set BoundVar)
  deriving (Eq, Show)
...
newtype Unify primTy primVal a = U (UnifyAlias primTy primVal a)
  ...
  deriving
    ( HasSource "globals" (Core.Globals IR.T IR.T primTy primVal),
      HasReader "globals" (Core.Globals IR.T IR.T primTy primVal)
    )
...
type HasGlobals primTy primVal =
  HasReader "globals" (Core.Globals IR.T IR.T primTy primVal)
