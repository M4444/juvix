...
import Juvix.Core.Base.Types (Globals, pattern VFree)
...
type NGlobals = Globals IR.T IR.T Nat.Ty Nat.Val
...
isOutside :: NError -> Bool
isOutside = \case OutsideVariables _ -> True; _ -> False
...
coreUnifier =
  ...
  where
    globals =
      ...
      [ ("F", Core.GAbstract $ Core.Abstract "F" Core.GSAny (VStar (Core.U 0))),
        ("K", Core.GDataCon $ Core.DataCon "K" (VStar (Core.U 0)) Nothing)
      ]
    α : β : _ = Meta.metaVars
    nfree = NFree . Global
    free = VFree . Global
