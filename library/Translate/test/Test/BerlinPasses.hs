module Test.BerlinPasses (top) where
...
sexpsByModuleTest =
  T.testGroup
    ...
    [ T.testCase "passing empty sexps evals to empty" $ do
        (defaultName, []) :| [] T.@=? Contextify.sexpsByModule defaultName [],
      T.testCase "passing only in-package sexps are ignored" $ do
        let inPackage = Structure.InPackage "foo" |> Sexp.serialize
        (defaultName, []) :| [] T.@=? Contextify.sexpsByModule defaultName (take 10 (repeat inPackage)),
      T.testCase "sexps following an in-package sexp are collected into a module" $ do
        ...
        fromList [("foo", following)] T.@=? (Contextify.sexpsByModule defaultName (package : following)),
      T.testCase "in-package prefixes followed by in-package forms are ignored" $ do
        ...
        fromList [("foo2", following)] T.@=? (Contextify.sexpsByModule defaultName (package1 : package2 : following)),
      T.testCase "consecutive packages are captured" $ do
        ...
        fromList [("foo1", following), ("foo2", following)] T.@=? (Contextify.sexpsByModule defaultName (forms1 <> forms2))
    ]
...
inContextSexpsTest =
  T.testGroup
    ...
    [ T.testCase "empty sexps are ignored" $ do
        [fooPackage] T.@=? Contextify.inContextSexps ("foo", []),
      T.testCase "extracts a defsig-match and adds module prefix" $ do
        ...
        expected T.@=? Contextify.inContextSexps ("foo", [defSigMatch]),
      T.testCase "extracts a type name, constructor and adds module prefix" $ do
        ...
        expected T.@=? Contextify.inContextSexps ("foo", [sexpType]),
      T.testCase "declare is not emitted" $ do
        ...
        expected T.@=? Contextify.inContextSexps ("foo", [defSigMatch, declare]),
      T.testCase "open is not emitted" $ do
        ...
        expected T.@=? Contextify.inContextSexps ("foo", [defSigMatch, open])
    ]
  where
