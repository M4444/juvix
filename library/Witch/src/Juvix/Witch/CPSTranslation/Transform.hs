...
mkTrans :: ExpressionIO m
        => Context.T Sexp.T ty sumRep -> [Sexp.T] -> Stack -> CPSTrans m ty sumRep
...
-- convert all vias
convertVias :: ExpressionIO m
            => ContextifiedProgram m ty sumRep -> ContextifiedProgram m ty sumRep
...
addHandlersToContext :: ExpressionIO m
                     => Context.T Sexp.T ty sumRep -> [Sexp.T] -> ContextifiedProgram m ty sumRep
...
construct sexp ctx
  | Just var <- Str.toVar sexp =
      extractValue <$> Context.lookupCurrent (Str.varName var) ctx
  | otherwise = Just sexp
  where
...
convert sexp form = do
  ...
  where
    conv ctx stack sexp1
      ...
      | Just lam_ <- Str.toLam sexp = do
         (st, sexp2) <- convertLam lam_ ctx
         pure (ctx, sexp2 : sexp1, st)

      | Just (Str.Let {letBinder = Str.Binder {..}, letBody = letBody}) <- Str.toLet sexp = do
         Just binder <- pure (construct binderTerm ctx)
         Just body <- pure (construct letBody ctx)
         (st, sexp2) <- convertLet (Str.Let (Str.Binder binderName binderUsage binder) body) (mkStkd stack ctx)
         pure (ctx, sexp2 : sexp1, st)

      | Just (Str.App {..}) <- Str.toApp sexp = do
         Just lambda <- pure (construct appFun ctx)
         Just args <- pure (construct appArg ctx)
         (st, sexp2) <- convertApp (Str.App lambda args) (mkStkd stack ctx)
         pure (ctx, sexp2 : sexp1, st)
...
convertLet :: ExpressionIO m
           => Str.Let
           -> Stacked m (Context.T Sexp.T ty sumRep)
           -> m (Stack, Sexp.T)
convertLet (Str.Let {letBinder = Str.Binder {..}, letBody = letBody}) form = do
      (k : ks, ctx) <- form
      ks' <- liftIO unique
...
convertApp :: ExpressionIO m
           => Str.App
           -> Stacked m (Context.T Sexp.T ty sumRep)
           -> m (Stack, Sexp.T)
convertApp (Str.App {..}) stkd = do
  (stk, context) <- stkd
...
convertLam :: ExpressionIO m
           => Str.Lam
           -> Context.T Sexp.T ty sumRep
           -> m (Stack, Sexp.T)
convertLam (Str.Lam {..}) context = do
  ks <- liftIO $ unique
...
convertHandler sexp acc
 ...
 | otherwise = do
  ...
  where
    convertHandler' :: ExpressionIO m
                    => Str.Handler -> Stacked m (Context.T Sexp.T ty sumRep) -> m Stack
